...
public class EnregistrementServiceImpl implements EnregistrementService {
    ...
    public Element enregistreUser(User user) {
        ...
        if (!user.mailCheck()) {
            ...
            return writer.createEnregistrementResponse("KO", 110, "mail non @univ-tlse3");
        }
        if (!user.alreadyExist()) {
            System.err.println("\t\t100 mail deja utilisé");
            return writer.createEnregistrementResponse("KO", 100, "mail deja utilise");
        }
        if (!user.OSMFound()) {
            System.err.println("\t\t200 adresse postale non reconnu");
            return writer.createEnregistrementResponse("KO", 200, "adresse postale non reconnu");
        }
